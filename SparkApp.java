 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> rA = rdd.filter(t -> t.getString(ORIGIN_CITY_NAME).equals("Seattle, WA"))
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> rB = d.filter(d.col("cancelled").equalTo('0'))
                .distinct()
                .groupBy(d.col("origincityname"), d.col("month"))
                .agg((functions.count("*").as("count")));
        Dataset<Row> rB2 = rB.select(rB.col("origincityname"), rB.col("month"), rB.col("count"));
        return rB2.javaRDD();
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> rC = d.filter(d.col("depdelayminutes").isNotNull())
                .distinct()
                .groupBy(d.col("origincityname"))
                .agg(functions.avg("depdelayminutes").as("avg"));
        Dataset<Row> rC2 = rC.select(rC.col("origincityname"), rC.col("avg"));
        return rC2.javaRDD();
    }
